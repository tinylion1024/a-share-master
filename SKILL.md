...
└────────┬──────────┘
    ...
    └────┬────┘
         ...
         ▼
┌───────────────────┐
│ ② 四维分析        │ ← references/four-dimensions.md
│   消息/情绪/技术   │
│   /风险            │
└────────┬──────────┘
         │
...
└────────┬──────────┘
    ...
    └────┬────┘
         │
    ┌────┴─────────────────────────────┐
    │ 评级 ≥ ⭐⭐⭐ → 输出操作报告    │ ← references/templates.md
    │ 评级 < ⭐⭐ → 输出观望/不买     │
    └─────────────────────────────────┘
```
...
用户问"有什么好股"
        ...
        ▼
┌───────────────────┐
│ ① 大盘环境判断    │ ← references/four-dimensions.md
│   上涨/震荡/下跌   │
└────────┬──────────┘
         │
         ▼
┌───────────────────┐
│ ② 筛选候选股票    │ ← references/scenes.md
│   趋势+流动性      │
│   +催化剂          │
└────────┬──────────┘
         │
...
└────────┬──────────┘
         ...
         ▼
┌───────────────────┐
│ ④ 输出机会清单    │ ← references/templates.md
│   排序+理由+仓位   │
└───────────────────┘
```
...
买入前检查清单
        ...
        ▼
┌───────────────────┐
│ ✓ 风控扫描通过    │ ← references/rules.md
│ ✓ 买入价确定      │ ← references/anchors.md
│ ✓ 止损价确定      │
│ ✓ 仓位计算完成    │
│ ✓ T+1规则确认     │
└────────┬──────────┘
...
│ 持仓监控          │
│ 每日检查：         │
│ - 价格 vs 止损    │ ← references/anchors.md
│ - 仓位是否超标    │
│ - T+1规则遵守     │
└────────┬──────────┘
         │
...
收盘后
        ...
        ▼
┌───────────────────┐
│ ① 今日操作复盘    │ ← references/scenes.md (每日复盘)
│   记录操作结果     │
└────────┬──────────┘
         │
         ▼
┌───────────────────┐
│ ② 评级验证        │ ← references/rating.md
│   预测 vs 实际     │
└────────┬──────────┘
         │
         ▼
┌───────────────────┐
│ ③ 误差分析        │ ← references/feedback-loop.md
│   发现规律/错误    │
└────────┬──────────┘
         │
         ▼
